...
class ApplicationStartupFileWatchConfiguration {
    ...
    public FileWatchServiceCustomizer fileWatchServiceCustomizer(FileWatchProperties fileWatchProperties) {
        return fileWatchService -> {
            ...
            if (!elements.isEmpty()) {
                fileWatchService.registerListener(new ApplicationStartupFileWatchListener(elements));
                for (StartupJarElement jarElement : elements) {
                    fileWatchService.registerWaitConfiguration(jarElement);
                }
            }
        };
    }
