...
/**
 * Default properties-based serializer for {@link NacosConfigTaskElement}.
 *
 * This implementation serializes a list of {@link NacosConfigTaskElement} into standard Java
 * properties format and deserializes it back. It processes non-static fields declared in the
 * superclass of {@link NacosConfigTaskElement}, preserving their declaration order.
 *
 ...
 * @since 3.0.2
 */
public class DefaultPropertiesNacosConfigTaskElementSerializer implements NacosConfigTaskElementSerializer {

    /** Prefix used to identify each element in the configuration. */
    private static final String PREFIX = "elements";

    /**
     * A map of non-static fields from the parent class of {@link NacosConfigTaskElement},
     * ordered by declaration sequence. Field accessibility is pre-enabled.
     */
    private static final Map<String, Field> FIELD_MAP
            = Arrays.stream(NacosConfigTaskElement.class.getSuperclass().getDeclaredFields())
    ...
    public String serialize(@NotNull List<NacosConfigTaskElement> elements) throws IOException {
        ...
        for (int i = 0; i < elements.size(); i++) {
            NacosConfigTaskElement element = elements.get(i);
    ...
    public List<NacosConfigTaskElement> deserialize(@NotNull String configInfo) throws IOException {
        ...
        List<NacosConfigTaskElement> elements = new ArrayList<>();
        ...
        try (BufferedReader reader = new BufferedReader(new StringReader(configInfo))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                while (elements.size() <= index) {
                    elements.add(new NacosConfigTaskElement());
                }
    ...
    /**
     * The value conversion of field types should be compatible with {@link NacosConfigTaskElement}.
