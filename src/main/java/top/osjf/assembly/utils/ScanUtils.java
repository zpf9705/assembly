...
public abstract class ScanUtils {
     ...
     * {@link top.osjf.assembly.support.SourceEnvironmentPostProcessor}.
     * Before initializing the spring boot container, please refer to
     * {@link top.osjf.assembly.support.SourceEnvironmentPostProcessor} and learn about
     * {@link org.springframework.boot.env.EnvironmentPostProcessor}
     *
     * @param source {@link SpringApplication#getAllSources()}
     */
    public static void applicationSource(@NonNull Set<Object> source) {
        if (source.isEmpty()) {
            throw new UtilException("No detection of the existence of the startup main class");
        }
    ...
    public static <T> Set<Class<T>> getSubTypesOf(Class<T> clazz, String... packageNames) {
        ...
        for (String packageName : packageNames) {
            ...
            if (CollectionUtil.isEmpty(scan)) {
                finder.addAll(scan);
            }
        }
        return finder;
            ...
            while (dirs.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    try {
                                ...
                                if ((idx != -1) || recursive) {
                                    ...
                                    if (name.endsWith(".class") && !entry.isDirectory()) {
                                        ...
                                        try {
                                            ...
                                            throw new UtilException("Error adding user defined view class. " +
                        ...
                        throw new UtilException("Error retrieving files from jar package " +
    ...
    private static <T> void scanByFile(String packageName, String packagePath, final boolean recursive,
        ...
        if (Objects.isNull(dirFiles) || ArrayUtil.isEmpty(dirFiles)) {
            return;
        }
        // Loop All Files
