...
public class Spectator<T> implements Observer<T>, Serializable {
    ...
    public Spectator(Supplier<T> run,
        ...
        Assert.notNull(run, "Run no be null");
        Assert.notNull(type, "Type no be null");
        Assert.notNull(check, "Check no be null");
        Assert.notNull(simpleMsgHandler, "SimpleMsgHandler no be null");
        this.run = run;
        this.type = type;
        this.check = check;
    ...
    public void accept(Consumer<T> consumer, Consumer<? super Throwable> exConsumer) {
        Assert.notNull(this.flowable,"Flowable no be null , " +
